...
@section('side-bar-nav')
    ...
    <li class="treeview ">
     ...
     <li class="treeview active">
        ...
        <ul class="treeview-menu">
            <li > <a href="{{ route('admin.home.nav','student-add-ui') }}"><i class="fa fa-plus text-green"></i> add</a></li>
            <li class="active"><a href="{{ route('admin.home.nav','student-undergrad-view') }}"><i class="fa fa-eye text-aqua"></i>undergraduate </a></li>
...
@section('breadcrumb')
    <h1>
        Students
        <small>(View Undergraduate Students)</small>
      </h1>
      <ol class="breadcrumb">
                        ...
                        <div class="table-responsive">
                         <br>
                            ...
                            <table class="table table-striped b-t b-light">
                                ...
                                <tbody>
                                   @foreach ( $undergrad_students as $undergrad_student)
                                        <tr>
                                        {{-- <td><label class="i-checks m-b-none"><input type="checkbox" name="post[]"><i></i></label></td> --}}
                                        <td><span class="text-ellipsis"> {{ $undergrad_student->id }}</span></td>
                                        <td><span class="text-ellipsis"> {{ $undergrad_student->fullname }}</span></td>
                                        <td><span class="text-ellipsis"> {{ $undergrad_student->id_number }}</span></td>
                                       

                                        <td><span class="text-ellipsis">
                                          @foreach ( $levels as  $level )
                                              @if( $undergrad_student->level_id == $level->id ) 
                                               {{ $level->number }}
                                              @endif
                                          @endforeach
                                        </span>
                                        ...
                                        <td><span class="text-ellipsis"> {{ $undergrad_student->email }}</span></td>
                                       
                                       
                                        <td><span class="text-ellipsis">
                                          @foreach ( $departments as  $department )
                                              @if( $undergrad_student->department_id == $department->id ) 
                                               {{ $department->name }}
                                              @endif
                                          @endforeach
                                        </span>
        ...
        if( $('#search') ){
                ...
                $('#search').on('keyup',function()
                {
                    ...
                    if($value .trim() !== '')
                    {
                        ...
                        url : '/admin/search/undergrad',

                        data:{search: $value, search_option :  $search_option, kind: 1},
